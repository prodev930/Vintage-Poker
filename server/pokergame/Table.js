...
class Table {
  constructor({
    ...
    this.seats = seats || this.initSeats(maxPlayers);
    this.board = board || [];
    this.deck = deck || null;
    this.turn = turn || null;
    this.pot = pot || 0;
    this.mainPot = mainPot || 0;
    this.callAmount = callAmount || null;
    this.minBet = minBet || this.limit / 200;
    this.minRaise = minRaise || this.limit / 100;
    this.smallBlind = smallBlind || null;
    this.bigBlind = bigBlind || null;
    this.handOver = handOver || true;
    this.winMessages = winMessages || [];
    this.wentToShowdown = wentToShowdown || false;
  ...
  nextUnfoldedPlayer(player, places) {
    ...
    while (i < places) {
      ...
      if(current>this.players.length) current=1;
  ...
  nextActivePlayer(player, places) {
    ...
    while (i < places) {
      let seat = this.seats[current];
      current = current === this.maxPlayers ? 1 : current;
  ...
  endWithoutShowdown() {
    ...
    winner &&
      ...
      tableController.gameresultState(winner.player.name, this.pot.toFixed(2));
      console.log("winner, amount", winner.player.name, this.pot.toFixed(2));
    this.endHand();
  }
  resetEmptyTable() {
    this.button = null;
