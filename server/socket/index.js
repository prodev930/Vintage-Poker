...
const init = (socket, io) => {
  ...
  socket.on(CREATE_TABLE, ({ id, maxPlayers, name, limit }) => {
    let _newTable = new Table({ id, name, limit, maxPlayers });
    tableController.createTable(id, maxPlayers, name, limit);
    const obj = {[id] : _newTable}
    tables = { ...tables, ...obj};
    
    io.sockets.emit(TABLE_CREATED, tables);
  });
  ...
  socket.on(JOIN_TABLE, (tableId) => {
    ...
    tableController.playerState(table, player);
...
(async () => {
  let list = await tableController.getTableList();
  if (list.status) {
    list.tables.forEach((table) => {
      tables[table.id] = new Table(table);
    });
  }
})();
async function getGameInitState(req, res) {
  try {
...
module.exports = { init, getGameInitState };
